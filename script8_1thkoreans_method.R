...
for (nsel in seqnsel)	{
  ...
  vtrainlist[[ciclosel]]<-predict(outRF,type='vote')
  vtargetlist[[ciclosel]]<-predict(outRF,newdata=testtarget,type='Vote')
  voutlist[[ciclosel]]<-predict(outRF,newdata=testout,type='vote')
...
for(c in 1:ciclosel){
  ...
  for(l in 1:(length(ntr)-1)){
    Vi<-c(Vi,sum(vtrainlist[[c]][(ntr[l]+1):ntr[l+1],l])/a[l])
  }
  Vlist[[c]]<-Vi
}
...
for(c in 1:ciclosel){
 decidedj<-NULL
 vteststar[[c]]<-rbind(vtargetlist[[c]],voutlist[[c]])
 decidedj<-vteststar[[c]]<alpha*Vlist[[c]]
...
par(mfrow=c(2,3))
plot(vteststar[[14]][,1:2],col=colors2,ylab = 'confidence for class 3',
     xlab = 'confidence for class 1',main = 'Confidence Distribution')
legend("topright", c('breed 1','breed 3','breed 4','novelty'), title='classes',pch=1,
       col=c('red3','royalblue3','yellowgreen','darkgrey'))

plot(vteststar[[14]][,c(1,3)],col=colors2,ylab = 'confidence for class 4',
     xlab = 'confidence for class 1',main = 'Confidence Distribution')
legend("topright", c('breed 1','breed 3','breed 4','novelty'), title='classes',pch=1,
       col=c('red3','royalblue3','yellowgreen','darkgrey'))

plot(vteststar[[14]][,c(2,3)],col=colors2,ylab = 'confidence for class 4',
     xlab = 'confidence for class 3',main = 'Confidence Distribution')
legend("topright", c('breed 1','breed 3','breed 4','novelty'), title='classes',pch=1,
       col=c('red3','royalblue3','yellowgreen','darkgrey'))

plot(vteststar[[14]][,1:2],col=colors1,ylab = 'confidence for class 3',
     xlab = 'confidence for class 1',main = 'Confidence Distribution')
legend("topright", c('known breeds','novelty'), title='classes',pch=1,
       col=c('darkorange2','darkgrey'))

plot(vteststar[[14]][,c(1,3)],col=colors1,ylab = 'confidence for class 4',
     xlab = 'confidence for class 1',main = 'Confidence Distribution')
legend("topright", c('known breeds','novelty'), title='classes',pch=1,
       col=c('darkorange2','darkgrey'))

plot(vteststar[[14]][,c(2,3)],col=colors1,ylab = 'confidence for class 4',
     xlab = 'confidence for class 3',main = 'Confidence Distribution')
legend("topright", c('known breeds','novelty'), title='classes',pch=1,
       col=c('darkorange2','darkgrey'))