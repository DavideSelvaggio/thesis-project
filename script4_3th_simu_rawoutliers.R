### soglie stabilite utilizzando le misure grezze di outlyingness
...
razzajtarget<-as.factor(rep((1:nr)[-c(esclusa,esclusaOut)],times=numtarget))
...
ind<-cumsum(c(150,300,675))
plot(density(rawoute[[1]][1,1:ind[1]]),col='red',ylim=c(0,0.2),xlim=c(66,1000))
lines(density(rawoute[[1]][1,(ind[1]+1):ind[2]]),col='blue')
lines(density(rawoute[[1]][1,(ind[2]+1):ind[3]]),col='orange')
lines(density(rawoute[[1]][2,(ind[1]+1):end]),col='green')
lines(density(rawoute[[1]][2,1:ind[1]]),col='violet')
lines(density(rawoute[[1]][2,(end+1):end2]),col='yellow')
...
min<-min(apply(rawoute[[1]],1,summary)[2,])
max<-max(apply(rawoute[[1]],1,summary)[5,])
for(u in 1:numiniz){
...
for (u in 1:numiniz){
  print(paste("c=", 1, "u=", u))
  outmin<-optim(inizpar[u,],funobb,soute=rawoute[[1]],cl=cl[[1]])
...
plot(rawoute[[1]][1,(ind[2]+1):ind[3]],col='grey',main='Distribution of class raw outlier
     and novelty outlier',ylab='raw outlyingness value',xlim=c(1,300),ylim=c(-2,10000))
points((rawoute[[1]][1,(ind[1]+1):ind[2]]),col='blue')
points((rawoute[[1]][1,1:ind[1]]),col='red')
legend("topright", c('class outlier ','other class', 'novelty'), title='class 2', pch=1,
       col=c('red','blue','grey'))
abline(h=optpar[[1]][1])


plot(rawoute[[1]][2,(ind[2]+1):ind[3]],col='grey',main='Distribution of class raw outlier
     and novelty outlier',ylab='raw outlyingness value',xlim=c(1,150),ylim=c(-2,5000))
points((rawoute[[1]][2,(ind[1]+1):ind[2]]),col='blue')
points((rawoute[[1]][2,1:ind[1]]),col='red')
legend("topright", c('class outlier ','other class', 'novelty'), title='class 3', pch=1,
       col=c('blue','red','grey'))
abline(h=optpar[[1]][2])
...
plot(rawoute[[1]][1,1:ind[1]],rawoute[[1]][2,1:ind[1]],col='red',main='Distribution of raw outlier
     between classes',xlab='outlyingness value class 2',ylab='outlyingness value class 3',xlim=c(0,4500),ylim=c(0,4500))
points(rawoute[[1]][1,(ind[1]+1):ind[2]],rawoute[[1]][2,(ind[1]+1):ind[2]],col='blue')
points(rawoute[[1]][1,(ind[2]+1):ind[3]],rawoute[[1]][2,(ind[2]+1):ind[3]],col='grey')
abline(v=optpar[[1]][1])
abline(h=optpar[[1]][2])
...
decided<-decision(rawout,optpar[[1]][1:2])
real<-c(rep("T",sum(numtarget)),rep("O",(dim(rawout)[2]-sum(numtarget))))
...
index1<-cumsum(c(numtarget,(dim(rawout)[2]-sum(numtarget))))
index2<-cumsum(as.numeric(table(decided)))
plot(rawout[1,(index1[1]+1):index1[2]],col='blue',ylim=c(-3,17500),ylab='raw outlyingness value',main='Distribution of raw outlier
     complete training')
points(rawout[1,(index1[2]+1):index1[3]],col='grey')
points(rawout[1,1:index1[1]],col='red')
legend("topright", c('class 2 ','class 3','novelty'), title='class 2', pch=1,
       col=c('red','blue','grey'))
abline(h=optpar[[1]][1])

##detail 1
plot(rawout[1,(index1[1]+1):index1[2]],col='blue',ylim=c(125,650),xlim=c(0,100),ylab='raw outlyingness value',main='Distribution of raw outlier
     complete training -Detail 1-')
points(rawout[1,(index1[2]+1):index1[3]],col='grey')
points(rawout[1,1:index1[1]],col='red')
legend("topright", c('class 2 ','novelty'), title='class 2', pch=1,
       col=c('red','grey'))
abline(h=optpar[[1]][1])