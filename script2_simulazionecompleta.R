...
fnew<-function(thr,out,razzajtarget){   #questa funzione dovrebbe lavorare sul parametro alpha, che influenza la soglia di accettazione/rifiuto di un elemento in una classe.
  r<-as.numeric(levels(razzajtarget))
  nr<-length(r)
  ss1<-matrix(1,nr,1)
  sens<-matrix(0,nr,1)
  spec<-matrix(0,nr,1)
  ss<-1
  for (i in 1:nr){
    spliti<-matrix(FALSE,nrow(out),1)
    spliti<-out[,i]<thr[i]
    true<-razzajtarget==r[i]
    if (length(unique(spliti))==2) {
      confmat<-table(true,spliti)
      sens[i]<-confmat[2,2]/sum(confmat[2,])
      spec[i]<-confmat[1,1]/sum(confmat[1,])
      #ss1[i]<-(1-(sens*spec)/(sens+spec))
    }
  }
  if (mean(sens)!= 0 & mean(spec)!= 0) ss<-(1-(mean(sens)*mean(spec))/(mean(sens)+mean(spec)))
  #mean(ss1)
  ss
}# fine function fnew
...
##accorpamento delle matrici soute
...
plot(soutematrixc[1:index1[3]],col='orange',xlim=c(0,500),ylim=c(-1,60),ylab='outlyingness value',main='Distribution of standardized outlier
     complete training
     -unique classes-')
points(soutematrixc[(index1[3]+1):index1[4]],col='grey')
legend("topright", c('classes outlier ','novelty'), pch=1, title='classes',
       col=c('orange','grey'))
abline(h=optpar$minimum)
